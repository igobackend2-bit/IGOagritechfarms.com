...
const HeroSection = () => {
  ...
  // Fit the hero section's own aspect ratio to whichever slide image is
  // currently showing, so `object-contain` never has to add black
  // letterboxing (and never crops) — works automatically for any image
  // shape, so future banner swaps don't need a code change.
  const [ratio, setRatio] = useState(16 / 9);
  const imgRefs = useRef<(HTMLImageElement | null)[]>([]);
  const applyRatioFromImg = useCallback((img: HTMLImageElement | null) => {
    if (img && img.naturalWidth && img.naturalHeight) {
      setRatio(img.naturalWidth / img.naturalHeight);
    }
  }, []);
  useEffect(() => {
    applyRatioFromImg(imgRefs.current[current]);
  }, [current, applyRatioFromImg]);
